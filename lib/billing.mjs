...
const GOOGLE_PLACES_TIMEOUT_MS = 9000;
...
async function findGooglePlace({ countryCode, countryName, region, category, googleMapsApiKey }) {
  ...
  try {
    ...
    const wrappedError = new Error(`Google Places API request failed: ${error.message}`);
    wrappedError.statusCode = 502;
    wrappedError.publicMessage = error.name === "AbortError"
      ? "Google Maps place lookup timed out. Please try again, or check whether Vercel can reach Google Places API."
      : "Google Maps place lookup failed. Vercel could not reach Google Places API.";
    throw wrappedError;
  } finally {
  ...
  if (!response.ok) {
    ...
    error.publicMessage = googlePlacesPublicMessage(response.status, detail);
    throw error;
  }
...
function googlePlacesPublicMessage(status, detail) {
  const normalizedDetail = String(detail || "").toLowerCase();
  const baseMessage = "Google Maps place lookup failed.";

  if (status === 400 && normalizedDetail.includes("places api")) {
    return `${baseMessage} Please enable Places API (New) for this Google Cloud project.`;
  }

  if (status === 400 && normalizedDetail.includes("api key not valid")) {
    return `${baseMessage} The Google Maps API key is not valid. Please paste the full key again.`;
  }

  if (status === 403) {
    if (normalizedDetail.includes("billing")) {
      return `${baseMessage} Please enable billing for the Google Cloud project that owns this API key.`;
    }

    if (normalizedDetail.includes("referer") || normalizedDetail.includes("referrer") || normalizedDetail.includes("ip")) {
      return `${baseMessage} This key is being used by the Vercel server, so browser HTTP referrer restrictions will not match. Use an unrestricted test key, a server-side key restriction that works for your hosting setup, or move the Places request to the browser.`;
    }

    return `${baseMessage} Check that the key has Places API (New) access, billing is enabled, and application restrictions allow calls from the Vercel server.`;
  }

  if (status === 429) {
    return `${baseMessage} Google Maps quota or rate limits were exceeded.`;
  }

  return `${baseMessage} Check your API key, billing, and Places API access.`;
}
